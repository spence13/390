...
void adcBufferInit()
{
    adcBuffer.indexIn = 0;//init the index to to zero
    adcBuffer.indexOut = 0;// init the index out to zero
    adcBuffer.elementCount = 0;// init the element count to zero
}
...
{
    uint32_t returnValue = 0;//create variable to hold return value
...
{
    return adcBuffer.elementCount;//return the element count
}

void isr_function()
{
    uint32_t adcData = interrupts_getAdcData();//save the adcdata
    isr_addDataToAdcBuffer(adcData);//add the adc data to buffer
    transmitter_tick();//tick the transmitter SM
    trigger_tick();//tick the trigger SM
    lockoutTimer_tick();//tick the lockout SM
    hitLedTimer_tick();//tick the led SM
}
